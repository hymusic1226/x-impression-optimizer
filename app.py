...
このアカウントの現状を100点満点で厳格に採点してください。
採点基準は以下の通りです（各20点満点）：
- Bio・プロフィールの最適化度
- 投稿頻度・一貫性
- エンゲージメント率（インプレッション対比）
- ハッシュタグ・SEO戦略
- コンテンツの差別化・独自性

【重要な採点ルール】
- フォロワー数が少ない・インプレッションが低い場合は厳しく減点する
- 情報が未入力の項目は「改善余地あり」として低めに採点する
- 一般的なアカウントの平均点は35〜50点程度とし、60点以上は明確な強みがある場合のみ
- 甘い採点は禁止。ユーザーが本当に改善すべき点を正確に伝えることが目的

スコアは必ず「**総合スコア：XX点 / 100点**」という形式で書いてください。
各項目の内訳点数と、スコアを下げた主な理由を具体的に列挙してください。
